<template>
    <div class="chat-area flex flex-col h-full bg-gray-900 text-white">
        ...
        <div ref="messageListRef" class="flex-1 overflow-y-auto p-4 space-y-3">
            ...
            <div
                v-if="chatStore.partialResponse"
                class="max-w-[80%] rounded-lg px-4 py-2 bg-gray-700 mr-auto text-white"
            >
            ...
            <div
                ...
                class="max-w-[80%] rounded-lg px-4 py-2 bg-red-700 mr-auto text-white"
            >
                <div v-for="(err, i) in errorMessages" :key="i" class="text-sm">
                    {{ err }}
                </div>
            </div>
        </div>
        ...
        switch (msg.type) {
            ...
            case "audio":
                ...
                try {
                    ...
                    const audioBlob = new Blob([new Uint8Array(chunks)], {
                        type: "audio/wav",
                    });
...
function onPTTEnd() {
    ...
    if (recorder && recorder.state !== "inactive") {
        ...
        recorder.onstop = () => {
            ...
            const blob = new Blob(recorder.stream.getTracks().length > 0 ? [] : [], {
                type: "audio/webm",
            });
