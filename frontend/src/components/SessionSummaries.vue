<template>
  <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
    ...
    <!-- No summaries yet -->
    <div v-if="!summaryStore.summaries.length && !summaryStore.generating" class="text-center py-8 text-gray-400">
      No summaries yet. Generate one to see structured session insights.
    </div>

    <!-- Summary cards -->
    <div v-for="(item, idx) in summaryStore.summaries" :key="item.id || idx" class="mb-6 rounded-xl border border-gray-200 bg-gray-50 p-5">
      <!-- Topics -->
      <div v-if="item.summary?.topics?.length" class="mb-4">
        <h3 class="mb-2 text-sm font-semibold uppercase tracking-wide text-gray-500">Topics</h3>
      ...
      <div v-if="item.summary?.new_vocabulary?.length" class="mb-4">
        <h3 class="mb-2 text-sm font-semibold uppercase tracking-wide text-gray-500">New Vocabulary</h3>
        <table class="min-w-full divide-y divide-gray-200 text-sm">
          ...
          <tbody class="divide-y divide-gray-200">
            <tr v-for="vocab in item.summary.new_vocabulary" :key="vocab.term">
              <td class="pr-4 py-2 font-medium text-gray-800">{{ vocab.term }}</td>
      ...
      <div v-if="item.summary?.mistakes?.length" class="mt-4">
        <h3 class="mb-2 text-sm font-semibold uppercase tracking-wide text-gray-500">Mistakes</h3>
        <div class="space-y-2">
          <details
          ...
          >
            <summary class="cursor-pointer list-none p-3 text-sm font-medium text-gray-700">
              {{ mistake.original }}
            </summary>
            <div class="border-t border-gray-200 p-3 text-sm text-gray-600">
              <p><span class="font-medium text-gray-700">Correction:</span> {{ mistake.correction }}</p>
              <p class="mt-1"><span class="font-medium text-gray-700">Explanation:</span> {{ mistake.explanation }}</p>
            </div>
          </details>
        </div>
      </div>
