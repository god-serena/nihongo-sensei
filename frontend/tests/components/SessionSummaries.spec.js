...
const mockSummary = {
  ...
  summary: {
    ...
    mistakes: [
      { original: '私は行きます学校', correction: '私は学校に行きます', explanation: 'Word order: destination + に + 行きます' },
    ],
  },
}
...
describe('SessionSummaries.vue', () => {
  ...
  it('disables the generate button while generating', async () => {
    let resolveGen
    mockCreateSummary.mockReturnValue(new Promise((resolve) => { resolveGen = resolve }))
    const wrapper = mountComponent()
